...
shinyServer(function(input, output) {
  ...
  output$equation <- renderText({
    req(round_data())
    if (round_data()$model == "exp") {
      eqn <- str_c("<pre>=if(<strong style=\"color: #00c\">A1</strong>&lt;", round_data()$low_score, ", 0, 100*exp(", round_data()$a, "*(<strong style=\"color: #00c\">A1</strong>-", round_data()$high_score, "))</pre>", sep = "")
